...
class ExerciseListTest(StaticLiveServerTestCase):
    ...
    def test_exercise_list(self):
        ...
        exercise_list_button = WebDriverWait(self.browser, 10).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, '[data-name="Exercises Page"]'))
        )
        
        exercise_list_button.click()
        time.sleep(5)
        
        self.browser.find_element(By.NAME, "searchBar").send_keys("Dumbbell Bent Over Row (Single Arm)")
        time.sleep(5)
        ...
        more_info_button = WebDriverWait(self.browser, 10).until(
            EC.element_to_be_clickable((By.NAME, "moreInfoButton"))
        )
